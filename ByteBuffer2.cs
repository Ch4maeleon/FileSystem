...
namespace MD.IO.Buffer
{
    ...
    public class ByteBuffer2
    {
        ...
        public DateTime GetDateTime6BDec()
        {
            ...
            string strDT = ToHex();
            long valid = 0;
            if (strDT.Length != 12 || string.IsNullOrEmpty(strDT) || !long.TryParse(strDT, out valid))
            {
                return res;
            }
        ...
        public ByteBuffer2 UpdateBuffer(byte[] buffer, int offset, int count)
        {
            if (buffer != null && count > 0)
            {
                ...
                {
                    System.Buffer.BlockCopy(buffer, offset, this.data, this.Offset, count);   // copy previous buffer
                }
                else
                {
        ...
        {
            return (b & 0x80) == 0 ? (byte)0 : (byte)0xFF;
        }
